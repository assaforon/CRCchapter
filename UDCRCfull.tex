...
Because UD sequences of assigned doses $x_1,\ldots,x_n$ possess the properties of a Markov chain, as $n\to\infty$ the relative proportions of subjects assigned to each dose in $\mathcal{X}$ approach the \emph{asymptotic allocation distribution} $\boldsymbol{\pi}=\left(\pi_1,\ldots,\pi_M\right)^{\prime};\ \ \sum_m \pi_m=1.$  If $\mathbf{P}$ is regular, then $\pi_m>0\ \forall m$. The distribution $\boldsymbol{\pi}$ can be found by solving $M$ linear equations with $M$ unknowns. These \emph{global balance equations} can be expressed in matrix form as $\boldsymbol{\pi}^\prime \mathbf{P} = \boldsymbol{\pi}^\prime$.

The tri-diagonal structure of $\mathbf{P}$ for first-order UD designs (\ref{eq:genericPmat}) greatly simplifies the global balance equations, which become
